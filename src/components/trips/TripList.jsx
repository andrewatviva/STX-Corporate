...
export function LeadTimeBadge({ days, showDays = false }) {
  if (days === null || days === undefined) return <span className="text-gray-500">—</span>;
  let label, cls;
      ...
      {extra > 0 && (
        <>
          ...
          {open && (
            ...
            >
              <p className="text-xs font-semibold text-gray-700 mb-2 uppercase tracking-wide">All passengers</p>
              <div className="flex items-center gap-2 text-sm text-gray-800">
                <span className="w-1.5 h-1.5 rounded-full bg-blue-500 shrink-0" aria-hidden="true" />
                {trip.travellerName}
                <span className="text-xs text-gray-600 ml-auto">Primary</span>
              </div>
              {trip.additionalPassengers.map((p, i) => (
                <div key={i} className="flex items-center gap-2 text-sm text-gray-700">
                  ...
                  {p.name}
                  {p.costCentre && <span className="text-xs text-gray-600 ml-auto truncate">{p.costCentre}</span>}
                </div>
              ))}
              <button
                type="button"
                onClick={() => setOpen(false)}
                className="mt-2 w-full text-xs text-gray-600 hover:text-gray-800 flex items-center justify-center gap-1"
              >
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        {hasFilters && (
          <button
            onClick={clearAll}
            className="flex items-center gap-1 text-xs text-gray-700 hover:text-gray-900 px-2 py-1 rounded hover:bg-gray-100 ml-auto"
          >
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
        <span className="text-xs font-medium text-gray-700 shrink-0">Travel dates:</span>
            ...
            <tbody>
              {filtered.map((trip, i) => (
                <tr
                  ...
                  onClick={() => onView(trip)}
                >
                  <td className="px-4 py-3 text-xs text-gray-600 font-mono hidden sm:table-cell whitespace-nowrap">
                    {trip.tripRef || '—'}
                  </td>
                  <td className="px-4 py-3 font-medium text-gray-800">
                    <span className="flex items-center gap-1.5">
                      ...
                      {(trip.attachments?.length ?? 0) > 0 && (
                        <Paperclip size={11} aria-hidden="true" className="text-gray-500 shrink-0" />
                      )}
                    </span>
                  </td>
                  ...
                  {isSTX && <td className="px-4 py-3 text-gray-700 text-xs">{trip.clientId || '—'}</td>}
                  <td className="px-4 py-3 text-gray-700">{trip.tripType || '—'}</td>
                  ...
                  <td className="px-4 py-3 text-right" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => onView(trip)} aria-label={`View trip: ${trip.title}`} className="text-blue-600 hover:text-blue-800 p-1 rounded">
                        <Eye size={14} aria-hidden="true" />
                      </button>
                      {canEditTrip && trip.status !== 'cancelled' && (
                        <button onClick={() => onEdit(trip)} aria-label={`Edit trip: ${trip.title}`} className="text-gray-600 hover:text-gray-900 p-1 rounded">
                          <Edit2 size={14} aria-hidden="true" />
                        </button>
                      )}
                      {isAdmin && (
                        <button onClick={() => onDelete(trip)} aria-label={`Delete trip: ${trip.title}`} className="text-gray-600 hover:text-red-600 p-1 rounded">
